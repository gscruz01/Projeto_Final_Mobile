...
class AutoresRepositoryImpl {
  final String baseUrl = 'http://10.0.2.2:3000';
  late final Endpoint endpoint;

  Future<List<AutoresModel>> getAutores() async {
    final response =
        await http.get(Uri.parse('$baseUrl${Endpoint.listAutores}'));

    if (response.statusCode == 200) {
      List<dynamic> jsonResponse = json.decode(response.body);
      return jsonResponse.map((item) => AutoresModel.fromJson(item)).toList();
    } else {
      throw Exception('Falha ao carregar autores');
    }
  }

  Future<AutoresModel> postAutores(String nome) async {
    ...
    if (response.statusCode == 201) {
      return AutoresModel.fromJson(json.decode(response.body));
    } else {
  ...
  Future<AutoresModel> putAutores(String id, String nome) async {
